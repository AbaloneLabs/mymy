...
fn cron_tool_schema() -> crate::agent::providers::ToolSchema {
    tool_schema(
        "cronjob",
        "Manage durable scheduled agent jobs. Every occurrence becomes a visible AgentRun.",
        serde_json::json!({
            "type": "object",
            "properties": {
                "action": { "type": "string", "enum": ["list", "create", "update", "pause", "resume", "remove", "trigger", "blueprints", "instantiate_blueprint"] },
                "id": { "type": "string" },
                ...
                "session_policy": { "type": "string", "enum": ["new", "reuse", "result_only"] },
                "catch_up_policy": { "type": "string", "enum": ["skip", "latest", "all"] },
                "retry_policy": { "type": "string", "enum": ["none", "safe"] },
                "max_tool_calls": { "type": "integer", "minimum": 1, "maximum": 1000 },
                "max_runtime_seconds": { "type": "integer", "minimum": 1, "maximum": 86400 },
                "max_total_tokens": { "type": "integer", "minimum": 1000, "maximum": 2000000 }
            },
            "required": ["action"]
        }),
    )
