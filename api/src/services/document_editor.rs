...
fn add_docx_image_replacements(
    original: &[u8],
    blocks: &mut [Value],
...
struct DocxImageData {
    bytes: Vec<u8>,
    extension: &'static str,
    mime_type: &'static str,
}

fn decode_docx_image_data_url(data_url: &str) -> AppResult<DocxImageData> {
    decode_ooxml_image_data_url(data_url, "DOCX")
}

fn decode_pptx_image_data_url(data_url: &str) -> AppResult<DocxImageData> {
    decode_ooxml_image_data_url(data_url, "PPTX")
}

fn decode_ooxml_image_data_url(data_url: &str, package_kind: &str) -> AppResult<DocxImageData> {
    ...
    let mime = header
        ...
        .unwrap_or_default();
    let Some((mime_type, extension)) = docx_image_mime_extension(mime) else {
        return Err(AppError::BadRequest(format!(
...
fn docx_image_mime_extension(mime: &str) -> Option<(&'static str, &'static str)> {
...
fn next_pptx_media_path(existing_names: &[String], extension: &str) -> String {
    let mut index = 1usize;
    loop {
...
fn docx_text_parts(bytes: &[u8], kind: &str) -> Vec<Value> {
...
fn add_docx_text_part_replacements(
    original: &[u8],
    value: Option<&Value>,
    kind: &str,
    replacements: &mut Vec<(String, Vec<u8>)>,
...
fn parse_docx_table_rows(table: &str) -> Vec<Vec<String>> {
    xml_segments(table, "<w:tr", "</w:tr>")
        .into_iter()
        .map(|row| {
            xml_segments(&row, "<w:tc", "</w:tc>")
                .into_iter()
                .map(|cell| extract_text_tags(&cell, "w:t").join(""))
                .collect::<Vec<_>>()
        })
...
fn docx_image_block_from_segment(
    segment: &str,
...
fn docx_image_relationship_id(segment: &str) -> Option<String> {
...
fn text_bytes(original: &[u8], model: &Value) -> AppResult<Vec<u8>> {
    let content = model
        .get("content")
        .and_then(Value::as_str)
...
fn validate_structured_text_for_path(path: &Path, bytes: &[u8]) -> AppResult<()> {
    let extension = path
        .extension()
        .and_then(|value| value.to_str())
        .unwrap_or_default()
...
fn validate_saved_document_bytes(
    kind: DocumentEditorKind,
    path: &Path,
    bytes: &[u8],
) -> AppResult<()> {
...
fn validate_ooxml_package(kind: DocumentEditorKind, bytes: &[u8]) -> AppResult<()> {
...
fn delimited_bytes(original: &[u8], model: &Value, delimiter: char) -> AppResult<Vec<u8>> {
    let rows = model
        .get("rows")
        .and_then(Value::as_array)
...
fn build_docx_image_paragraph(block: &Value) -> String {
...
fn build_docx_table(block: &Value) -> String {
    let rows = block
        .get("rows")
        .and_then(Value::as_array)
        .cloned()
