...
import { transformXlsxWorkbookReferencesForStructureEdit } from "./spreadsheetWorkbookReferences";
...
export function createSpreadsheetSheetStructureActions({
  ...
  function addRow() {
    if (!sheet || xlsxStructureEditBlockReason(sheet)) return;
    const insertAt = activeCell ? activeCell.row + 1 : sheet.rows.length;
    const nextRows = ensureXlsxRows(sheet, insertAt, columnCount).map(
      (row, rowIndex) => ({
        ...row,
    ...
    commitXlsxModel(
      transformXlsxWorkbookReferencesForStructureEdit(
          ...
          sheets: model.sheets.map((item) =>
            item.id === sheet.id
              ? {
                  ...
                  autoFilter: shiftXlsxRangeForRowInsert(
                    item.autoFilter,
                    insertAt,
                  ),
                }
        ...
        sheet.id,
        { axis: "row", kind: "insert", index: insertAt },
      ),
    );
  ...
  function addColumn() {
    ...
    commitXlsxModel(
      transformXlsxWorkbookReferencesForStructureEdit(
          ...
          sheets: model.sheets.map((item) =>
            item.id === sheet.id
              ? {
                  ...
                  autoFilter: shiftXlsxRangeForColumnInsert(
                    item.autoFilter,
                    insertAt,
                  ),
        ...
        sheet.id,
        { axis: "column", kind: "insert", index: insertAt },
      ),
    );
  ...
  function sortRowsByActiveColumn(direction: "asc" | "desc") {
    if (!sheet || !activeCell || !selectionRange) return;
    if (
      xlsxSortBlockReason(sheet, selectionRange, activeCell.column, filterText)
    ) {
      return;
    }
    const sorted = sortXlsxRange(
      sheet.rows,
      selectionRange,
      activeCell.column,
      direction,
  ...
  function deleteActiveRow() {
    if (
      ...
      sheet.rows.length <= 1 ||
      xlsxStructureEditBlockReason(sheet)
    ) {
      return;
    }
    commitXlsxModel(
      transformXlsxWorkbookReferencesForStructureEdit(
        ...
        sheet.id,
        { axis: "row", kind: "delete", index: activeCell.row },
      ),
    );
    setActiveCell(null);
  ...
  function deleteActiveColumn() {
    ...
    commitXlsxModel(
      transformXlsxWorkbookReferencesForStructureEdit(
          ...
          sheets: model.sheets.map((item) =>
            item.id === sheet.id
              ? {
                  ...
                  columns: shiftXlsxColumnsForDelete(
                    item.columns,
                    activeCell.column,
                  ),
        ...
        sheet.id,
        { axis: "column", kind: "delete", index: activeCell.column },
      ),
    );
    setActiveCell(null);
    setSelectionAnchor(null);
...
export function xlsxStructureEditBlockReason(sheet: XlsxSheet | undefined) {
  if (!sheet) return "Select a sheet before changing its structure";
  if (sheet.protection?.enabled) {
    return "Disable sheet protection before inserting or deleting rows and columns";
  }
  if ((sheet.pivots?.length ?? 0) > 0) {
    return "Pivot cache ranges are not represented, so structural edits are blocked";
  }
  return null;
}
