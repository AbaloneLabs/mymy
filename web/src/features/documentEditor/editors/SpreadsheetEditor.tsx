...
function evaluateConditionalFormula(
  ...
  try {
    ...
    const parser = new FormulaParser(expression, (reference) =>
      xlsxDisplayValueForRef(sheet, columnCount, reference),
    );
    return formulaValueBoolean(parser.parse());
  } catch {
    return false;
  }
}
...
function evaluateConditionalFormulaOperand(
  ...
  return formulaValueNumber(parser.parse());
}
...
function evaluateFormulaCachedValue(
  ...
  try {
    const parser = new FormulaParser(formula, valueForRef);
...
function xlsxFillInputFromCell(
  ...
  if (!cell) return "";
  if (cell.formula) {
    return `=${adjustFormulaReferences(cell.formula, rowOffset, columnOffset)}`;
  }
  return cell.value;
}
...
class FormulaParser {
  ...
  private parseComparison(): FormulaValue {
    ...
    while (
      ...
      value = compareFormulaValues(value, right, operator);
    }
    return value;
  }

  private parseConcat(): FormulaValue {
    let value: FormulaValue = this.parseAdditive();
    while (this.matchOperator("&")) {
      value = `${formulaValueText(value)}${formulaValueText(this.parseAdditive())}`;
    }
    return value;
  }
  ...
  private parsePower(): number {
    let value = formulaValueNumber(this.parseUnary());
    while (this.matchOperator("^")) {
      value = value ** formulaValueNumber(this.parseUnary());
    }
    return value;
  }

  private parseUnary(): FormulaValue {
    if (this.matchOperator("+")) return formulaValueNumber(this.parseUnary());
    if (this.matchOperator("-")) return -formulaValueNumber(this.parseUnary());
  ...
  private parsePrimary(): FormulaValue {
    ...
    if (token.type === "identifier") {
      ...
      if (isFormulaCellReference(token.value)) {
  ...
  private evaluateFunction(name: string): FormulaValue {
    ...
    const numbers = args.map(formulaValueNumber);
    ...
    if (upper === "COUNT") {
      return args.filter(
        (value) =>
          formulaValueText(value) !== "" && Number.isFinite(Number(value)),
      ).length;
    }
    if (upper === "COUNTA") return args.filter((value) => formulaValueText(value) !== "").length;
    ...
    if (upper === "IF") {
      return formulaValueBoolean(args[0]) ? (args[1] ?? true) : (args[2] ?? false);
    }
    if (upper === "AND") return args.every(formulaValueBoolean);
    if (upper === "OR") return args.some(formulaValueBoolean);
    if (upper === "NOT") return !formulaValueBoolean(args[0]);
    if (upper === "LEN") return formulaValueText(args[0]).length;
    if (upper === "CONCAT" || upper === "CONCATENATE") {
      return args.map(formulaValueText).join("");
    }
    if (upper === "LEFT") {
      return formulaValueText(args[0]).slice(0, Math.max(0, Math.trunc(numbers[1] ?? 1)));
    }
    if (upper === "RIGHT") {
      const text = formulaValueText(args[0]);
    ...
    if (upper === "LOWER") return formulaValueText(args[0]).toLowerCase();
    if (upper === "UPPER") return formulaValueText(args[0]).toUpperCase();
    if (upper === "TRIM") return formulaValueText(args[0]).trim().replace(/\s+/g, " ");
  ...
  private parseFunctionArgument(): FormulaValue[] {
    ...
    if (
      ...
      isFormulaCellReference(start.value) &&
      isFormulaCellReference(end.value)
    ) {
      this.advance();
      this.advance();
      this.advance();
      return formulaRangeReferences(start.value, end.value).map((reference) =>
        this.valueForRef(reference),
      );
    }
...
function formulaRangeReferences(startRef: string, endRef: string) {
  const start = formulaReferencePosition(startRef);
  const end = formulaReferencePosition(endRef);
  if (!start || !end) return [];
...
function formulaReferencePosition(reference: string): CellPosition | null {
  const match = /^([A-Z]+)(\d+)$/i.exec(normalizeFormulaRef(reference));
  if (!match) return null;
  return {
    row: Math.max(0, Number(match[2]) - 1),
    column: columnIndexFromName(match[1]),
...
function isFormulaCellReference(value: string) {
  return /^[A-Z]+\d+$/i.test(normalizeFormulaRef(value));
}
...
function formulaValueNumber(value: FormulaValue | string | undefined) {
  if (typeof value === "boolean") return value ? 1 : 0;
  const number = Number(value ?? "");
  return Number.isFinite(number) ? number : 0;
}

function formulaValueText(value: FormulaValue | string | undefined) {
  if (value === undefined) return "";
  if (typeof value === "boolean") return value ? "TRUE" : "FALSE";
  return String(value);
}

function formulaValueBoolean(value: FormulaValue | string | undefined) {
...
function compareFormulaValues(
  ...
  const comparable =
    Number.isFinite(leftNumber) && Number.isFinite(rightNumber)
      ? leftNumber - rightNumber
      : formulaValueText(left).localeCompare(formulaValueText(right), undefined, {
          numeric: true,
          sensitivity: "base",
        });
...
function formatFormulaResult(value: FormulaValue) {
  if (typeof value === "boolean") return value ? "TRUE" : "FALSE";
