...
function markdownOutline(content: string): MarkdownHeading[] {
...
function markdownStats(content: string, headings: number) {
...
function replaceFrontmatterBody(content: string, body: string) {
  const frontmatter = parseFrontmatter(content);
...
function parseFrontmatterFields(content: string, marker: "---" | "+++") {
...
function formatFrontmatterField(key: string, value: string, marker: "---" | "+++") {
  return marker === "+++" ? `${key} = ${value}` : `${key}: ${value}`;
}

function indentMarkdownLine(line: string) {
  return `  ${line}`;
}

function outdentMarkdownLine(line: string) {
  return line.replace(/^( {1,2}|\t)/, "");
}

function offsetForLine(content: string, line: number) {
  if (line <= 1) return 0;
  let offset = 0;
...
function nextMarkdownFootnoteId(content: string) {
...
function isMarkdownUrl(value: string) {
...
function isMarkdownHeadingKey(value: string): value is `${MarkdownHeadingLevel}` {
  return /^[1-6]$/.test(value);
}

function buildMarkdownSearchRegex(
  query: string,
...
function countMarkdownSearchMatches(
  content: string,
  query: string,
  options: { matchCase: boolean },
) {
...
function nextMarkdownSearchRange(
  content: string,
  query: string,
